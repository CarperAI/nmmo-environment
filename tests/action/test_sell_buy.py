...
class TestSellBuy(ScriptedTestTemplate):
  # pylint: disable=invalid-name,protected-access,multiple-statements
  ...
  def test_sell_buy(self):
    ...
    price_edges = action.Price.edges
    ...
    actions[ent_id] = { action.Sell: {
        ...
        action.Price: price_edges[price] } }
    ...
    for ent_id, price in listing_price.items():
      ...
      actions[ent_id] = { action.Sell: {
          action.InventoryItem: env.obs[ent_id].inventory.sig(*item_sig),
          action.Price: price_edges[price-1] } }
    ...
    actions[ent_id] = { action.Sell: {
        action.InventoryItem: env.obs[ent_id].inventory.sig(*item_sig),
        action.Price: price_edges[7] } } # try to set different price
